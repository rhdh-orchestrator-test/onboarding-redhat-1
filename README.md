...
## Running onboardingredhat1workflow in Quarkus dev mode
...
```
(onboardingredhat1workflow should be included in the returned array)
...
[onboardingredhat1workflow-schema.json](./src/main/resources/schemas/onboardingredhat1workflow-schema.json ), run the following to create an
instance of the onboardingredhat1workflow workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/onboardingredhat1workflow -d@input.json
```
